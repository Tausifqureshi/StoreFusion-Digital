...
function Filter() {
  const { mode, product, searchkey, setSearchkey, filterType, setFilterType, setFilterPrice } = useContext(MyContext);
  const [minPrice, setMinPrice] = useState('');
  const [maxPrice, setMaxPrice] = useState('');

  const handlePriceFilter = () => {
    setFilterPrice({ min: minPrice || 0, max: maxPrice || Infinity });
  };
  ...
  const resetFilter = () => {
    ...
    setMinPrice('');
    setMaxPrice('');
    setFilterPrice({ min: 0, max: Infinity });
  };
      ...
      >
        ...
        <div className="mt-4">
          <button
            ...
            className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-sm font-medium rounded-md transition duration-200"
          >
            Apply Price Filter
          </button>
        </div>
