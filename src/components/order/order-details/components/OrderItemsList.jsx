    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {order.cartItems?.slice(0, visibleCount).map((item, i) => (
          <div key={i} onClick={() => navigate("/productInfo/" + item.id)} className={`flex gap-5 items-center p-5 rounded-[30px] border cursor-pointer transition-all hover:shadow-lg ${isDark ? "bg-[#1a1f2e] border-gray-700 hover:border-blue-500/50" : "bg-white border-gray-100 hover:border-blue-200"}`}>
        ...
        <div className="flex gap-4 mt-6">
          {visibleCount < order.cartItems.length && (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isDark ? "bg-[#1a1f2e] text-white hover:bg-gray-700" : "bg-gray-100 text-gray-900 hover:bg-gray-200"}`}
            >
              See More
            </button>
          )}
          {visibleCount > 2 && (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isDark ? "bg-[#1a1f2e] border border-gray-700 text-gray-400 hover:text-white" : "bg-white border border-gray-200 text-gray-500 hover:text-gray-900 hover:bg-gray-50"}`}
            >
              See Less
            </button>
          )}
