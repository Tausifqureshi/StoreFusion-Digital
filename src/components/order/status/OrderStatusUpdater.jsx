...
const OrderStatusUpdater = ({ orderId, currentStatus, isDark, onClose }) => {
  const [loading, setLoading] = useState(null); // kon sa button loading me hai

  const handleStatusUpdate = async (newStatus) => {
    if (newStatus === currentStatus) return; // same status pe click → kuch mat karo
  ...
  return (
    <div className={`rounded-2xl p-4 border shadow-xl w-64 ${isDark ? 'bg-[#1e293b] border-gray-700' : 'bg-white border-gray-200'}`}>
      ...
      <div className="flex flex-col gap-2">
        {STATUS_BUTTONS.map(({ label, icon: Icon, color }) => {
          const isActive = currentStatus?.toLowerCase() === label; // current status highlight
