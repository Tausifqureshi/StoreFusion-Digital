...
function UserOrderItem({ order, isDark, navigate, setUpdatingOrderId }) {
  return (
    <div  
      ...
      className={`rounded-[30px] border-2 p-5 md:p-6 transition-all hover:shadow-xl cursor-pointer ${isDark ? "bg-[#1a1f2e] border-gray-800 shadow-none" : "bg-white border-gray-100 shadow-blue-500/5"}`}
    >
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <span className={`px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1.5 ${isDark ? "bg-[#1a1f2e] border border-gray-700 text-gray-300" : "bg-gray-50 border border-gray-200 text-gray-700"}`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-t pt-5 border-gray-200 dark:border-gray-800">
        ...
        <div className="flex gap-3">
          {order.cartItems?.slice(0, 2).map((item, i) => (
            <div key={i} className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl p-2 flex items-center justify-center border transition-all ${isDark ? "bg-[#1a1f2e] border-gray-700" : "bg-white border-gray-200 shadow-sm"}`}>
          ...
          {order.cartItems?.length > 2 && (
            <div className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl border flex items-center justify-center text-sm font-black ${isDark ? "bg-[#1a1f2e] border-gray-700 text-gray-300" : "bg-gray-50 border-gray-200 text-gray-600 shadow-sm"}`}>
              +{order.cartItems.length - 2}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col items-start md:items-end gap-5 w-full md:w-auto">
          <div className="flex flex-wrap gap-2 w-full justify-start md:justify-end md:mt-auto">
            <button onClick={(e) => { e.stopPropagation(); navigate(`/order-details/${order.id}`); }} className={`flex-1 md:flex-none px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 ${isDark ? "bg-[#1a1f2e] border border-gray-700 text-white hover:border-gray-500" : "bg-gray-100 text-gray-900 border border-transparent hover:bg-gray-200"}`}>
              View Details
            </button>
