...
const NavLinks = React.memo(function NavLinks({ isDark, navItems, user, totalOrders, handleNavigate }) {
  ...
  return (
    <div className={`hidden lg:flex items-center lg:gap-4 xl:gap-8 font-bold lg:text-[11px] xl:text-[13px] uppercase tracking-wider shrink-0 ${isDark ? "text-gray-200" : "text-gray-600"}`}>
      ...
      {user && (
        <button onClick={() => handleNavigate('/order')} className="relative flex items-center hover:text-orange-500 transition-colors uppercase">
          Orders {totalOrders > 0 && <span className="absolute -top-1 -right-3 bg-green-500 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold shadow-sm">{totalOrders}</span>}
        </button>
      )}
...
const CartCounter = React.memo(function CartCounter() {
  ...
  return totalQuantity > 0 ? (
    <span className="absolute -top-1.5 -right-1.5 bg-orange-500 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold shadow-sm">
      {totalQuantity}
    </span>
  ) : null;
  ...
  return (
    <div className="flex items-center gap-4 lg:gap-3 xl:gap-5 shrink-0">
      ...
      <div className="relative flex items-center">
        <button onClick={() => handleNavigate('/cart')} className="flex items-center">
          <FiShoppingCart size={22} className={isDark ? "text-white" : "text-gray-700"} />
        </button>
        <CartCounter />
      </div>
...
function Navbar({ isDark }) {
  ...
  // ✅ MOBILE NAV: Menu band hone ke baad redirect karne ke liye

  const [navUrl, setNavUrl] = useState(null);

  const delayedNav = useCallback((url) => {
    navigate(url);
  }, [navigate]);
  ...
  const handleMobileClick = useCallback((url) => {
    setOpen(false); // Menu close animation start
    setNavUrl(url); // Path queue mein daal do
  }, []);



  const handleLogout = useCallback(async () => {
    // 1. Redux state ko turant clean karo
    dispatch(setLoggedInUser(null));
    // 2. Firebase session end karo
    await authService.logoutUser();


    // 3. Poora page reload karo taaki memory cache flush ho jaye
          ...
          >
            <div className={`relative w-full h-full flex flex-col overflow-hidden ${isDark ? "bg-[#1a1f2e] text-white" : "bg-white text-gray-900"}`}>
              <div className={`p-6 flex items-center gap-4 shrink-0 ${isDark ? "bg-gray-800/40" : "bg-blue-600 text-white"}`}>
                ...
                <h2 className="text-lg font-bold italic flex-1">Hello, {user ? (user.fullName || user.name || "User").split(" ")[0] : "Sign In"}</h2>
                <button
                  onClick={() => {
                    setOpen(false);
