...
function ProductCategories({ isDark, products }) {
  const chartData = useMemo(() => {
    ...
    products.forEach((product) => {
      ...
      categoryCountMap[formattedCat] =
        (categoryCountMap[formattedCat] || 0) + 1;
    });
    ...
    const sortedCategories = Object.entries(categoryCountMap).sort(
      (a, b) => b[1] - a[1],
    ); // b[1] - a[1] = descending order (bada pehle)
    ...
    const finalPercentages = top5Categories.map((item) =>
      Math.round((item[1] / totalProducts) * 100),
    ); // percentage calculate karo
    const finalColors = top5Categories.map(
      (_, index) => baseColors[index % baseColors.length],
    ); // har category ko ek color do
  ...
  return (
    <div
      className={`p-6 sm:p-8 rounded-3xl border transition-all h-full flex flex-col ${isDark ? "bg-[#1a1f2e] border-gray-800 shadow-[0_8px_30px_rgba(0,0,0,0.3)]" : "bg-white border-gray-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)]"}`}
    >
          ...
          <div className="grid grid-cols-2 gap-y-3 gap-x-2 mt-8">
            {data.labels.map((label, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                ...
                ></span>
                <span
                  className={`font-bold truncate ${isDark ? "text-gray-300" : "text-gray-600"}`}
                  title={label}
                >
                  {label}
                </span>
                <span
                  className={`text-xs ml-auto ${isDark ? "text-gray-500" : "text-gray-400"}`}
                >
