...
import { MyContext } from "../../context api/myContext";
import Loader from "../../components/loader/Loader";
import { FaHeart, FaTruck, FaShieldAlt, FaSyncAlt, FaChevronDown, FaTag, FaCheckCircle } from "react-icons/fa";
import Testimonial from "../../components/testimonial/Testimonial";
import AddTestimonial from "../../components/testimonial/AddTestimonial";
...
function ProductInfo() {
  ...
  const handleViewAll = () => {
  // User ko category page par bhejne ke liye
  ...
  const gallery = [product.imageUrl, product.imageUrl2 || product.imageUrl, product.imageUrl3 || product.imageUrl];
      ...
      <div className={`min-h-screen py-6 lg:py-10 pt-24 lg:pt-32 transition-all ${isDark ? "bg-[#131921] text-white" : "bg-white text-gray-900"}`}>
        <div className="container mx-auto px-4 lg:px-20">
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-start justify-center">
            ...
            {/* --- RIGHT: PRODUCT INFO (CENTERED ON MOBILE) --- */}
            <div className="w-full lg:w-[50%] flex flex-col space-y-6 text-center lg:text-left">
              <div>
                ...
                <div className="flex items-center justify-center lg:justify-start gap-3">
                  <span className="bg-blue-600 text-white px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest">{product.category}</span>
              ...
              <div className="py-2 md:py-4 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-end justify-center lg:justify-start gap-3 mb-1">
                  <span className="text-3xl md:text-4xl font-black text-blue-600 italic">₹{finalPrice}</span>
                  {discount > 0 && <span className="text-lg md:text-xl line-through text-gray-400 font-bold mb-1">₹{product.price}</span>}
                </div>
                <p className="text-[9px] font-black text-orange-500 uppercase tracking-[0.2em]">Shipping calculated at checkout</p>
              </div>
              ...
              {/* ✅ Wapas Aaye Icons (Trust Badges) - Center Aligned */}
          ...
          {/* SIMILAR PRODUCTS (MINIMAL DESIGN) */}
          {/* {similarProducts.length > 0 && (
            <section className="mt-20 lg:mt-32">
              <div className="flex items-center justify-center lg:justify-start gap-4 mb-10">
                <h2 className="text-xl md:text-3xl font-black uppercase tracking-tighter whitespace-nowrap italic">You May <span className="text-blue-600">Also Like</span></h2>
                <div className="hidden md:block flex-1 h-[1px] bg-gray-100 dark:bg-gray-800"></div>
              </div>
              
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-10">
                {similarProducts.map((item) => (
                  <div key={item.id} onClick={() => navigate(`/productinfo/${item.id}`)} className="group cursor-pointer flex flex-col items-center text-center">
                    <div className={`aspect-square w-full mb-4 rounded-[30px] md:rounded-[40px] flex items-center justify-center p-6 md:p-10 transition-all duration-500 overflow-hidden ${isDark ? "bg-[#1e293b]" : "bg-gray-50 hover:bg-white hover:shadow-2xl"}`}>
                      <img src={item.imageUrl} alt={item.title} className="max-h-full object-contain transition-transform duration-500 group-hover:scale-110" />
                    </div>
                    <p className="text-[9px] font-black text-blue-600 uppercase mb-1">{item.category}</p>
                    <h3 className={`font-bold text-[11px] md:text-sm uppercase tracking-tighter truncate w-full px-2 ${isDark ? "text-gray-200" : "text-gray-800"}`}>
                      {item.title}
                    </h3>
                    <p className="text-blue-600 font-black text-sm md:text-base mt-1">₹{item.price}</p>
                  </div>
                ))}
              </div>
            </section>
...
{similarProducts.length > 0 && (
  <section className="mt-12 lg:mt-24 border-t border-gray-100 dark:border-gray-800 pt-8 px-2 md:px-0">
    ...
    <div className="flex items-center justify-between mb-6 lg:mb-10">
      ...
      {/* Mobile par ye button chota dikhega */}
      <button onClick={handleViewAll } className="text-[9px] md:text-[11px] font-black uppercase tracking-widest text-blue-600 border-b-2 border-blue-600 pb-0.5 hover:text-orange-500 hover:border-orange-500 transition-all">
        View All
      </button>
    </div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-4 lg:gap-6">
      {similarProducts.map((item) => (
        <div 
          key={item.id} 
          onClick={() => navigate(`/productinfo/${item.id}`)} 
          className={`group cursor-pointer p-3 md:p-4 border transition-all duration-300 relative flex flex-col h-full
            ${isDark ? "bg-[#131921] border-gray-800 hover:border-blue-600" : "bg-white border-gray-100 hover:shadow-xl hover:shadow-gray-200/50"}`}
        >
          ...
          {/* Product Details - Stacked for better readability */}
          <div className="flex-1 flex flex-col justify-between">
            <div>
              <p className="text-[7px] md:text-[9px] font-black text-orange-500 uppercase tracking-widest mb-1">
                {item.category}
              </p>
              <h3 className={`font-bold text-[10px] md:text-xs lg:text-sm uppercase tracking-tight line-clamp-2 leading-tight mb-2 ${isDark ? "text-gray-200" : "text-gray-800"}`}>
                {item.title}
              </h3>
            </div>
            
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mt-auto">
              <span className="text-blue-600 font-black text-xs md:text-base lg:text-lg italic">
                ₹{item.price}
              </span>
              
              {/* Badge: Desktop par bada, Mobile par icon-only ya chota */}
              <div className="flex items-center gap-1">
                ...
                <FaCheckCircle className="text-blue-600 shrink-0" size={12} />
              </div>
            </div>
          </div>

          {/* Mobile optimization: Touch area bada rakha hai */}
        </div>
      ))}
    </div>
  </section>
